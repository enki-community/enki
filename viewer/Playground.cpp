...
world(world)
{
	...
	#define PROBLEM_CENTER_THYMIO

	#ifdef PROBLEM_CENTER_THYMIO
	{
		thymio = new Thymio2;
		viewer->setMovableByPicking(thymio,true);
		thymio->pos = Point(0, 0);
//		thymio->leftSpeed = 4;
//		thymio->rightSpeed = 5;
		world->addObject(thymio);
	}
	#endif // PROBLEM_CENTER_THYMIO
	...
	#ifdef PROBLEM_GENERIC_TOY
	{
		...
		viewer->setMovableByPicking(o,true);
	...
	viewer->setMovableByPicking(marxbot,true);
	...
	resetButton = new QPushButton("reset");
		...
		connect(resetButton,SIGNAL(pressed()),this,SLOT(resetCallback()));
		resetButton->setStyleSheet("background-color:rgb(150,150,150);");
	trackballButton = new QPushButton("track");
		trackballButton->setEnabled(false);
		trackballButton->setFixedSize(100,30);
		connect(trackballButton,SIGNAL(pressed()),viewer,SLOT(toogleTrackball()));
		trackballButton->setStyleSheet("background-color:rgb(150,150,150);");
	...
	QWidget* widget2 = new QWidget();
		...
		widget2->setStyleSheet("background-color:rgb(200,200,200);");

	widget2->setLayout(toolbar);
	layout1->addWidget(widget2);
	toolbar->addWidget(resetButton);
	toolbar->addWidget(trackballButton);
...
{
	EPuck *epuck = new EPuck;
	viewer->setMovableByPicking(epuck,true);
...
void EnkiPlayground::timerEvent(QTimerEvent * event)
{
	...
	if(viewer->getSelectedObject())
	{
		trackballButton->setEnabled(true);
		if(viewer->isTrackballActivated()) trackballButton->setText("untrack");
		else trackballButton->setText("track");
	}
	else
	{
		trackballButton->setEnabled(false);
		trackballButton->setText("track");
	}
	time.restart();
}
...
void EnkiPlayground::resetCallback()
{
	for (World::ObjectsIterator it = world->objects.begin(); it != world->objects.end(); ++it)
	{
		std::map<PhysicalObject*,PhysicalObjectSave>::iterator obj = saveState.find(*it);
		if(obj != saveState.end())
		{
			...
			Thymio2* thymio = dynamic_cast<Thymio2*>(*it);;
			if (thymio)
...
void EnkiPlayground::saveWorld()
{
	saveState.clear();
	for (World::ObjectsIterator it = world->objects.begin(); it != world->objects.end(); ++it)
		saveState[(*it)] = PhysicalObjectSave((*it)->pos,(*it)->angle);
}
